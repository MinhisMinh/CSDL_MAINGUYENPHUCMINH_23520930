...
SELECT COUNT(*) AS SoLuongHoaDonKhongPhaiCuaKhachHangThanhVien
FROM HOADON
WHERE MAKH NOT IN(
	SELECT MAKH 
	FROM KHACHHANG 
...
SELECT THANG 
FROM (
	SELECT MONTH(NGHD) THANG, RANK() OVER (ORDER BY SUM(TRIGIA) DESC) RANK_TRIGIA 
	FROM HOADON
	WHERE YEAR(NGHD) = '2006' 
	GROUP BY MONTH(NGHD)
) A
...
INNER JOIN SANPHAM SP ON A.MASP = SP.MASP
WHERE RANK_SL = 1
GO

-- 43.	*Mỗi nước sản xuất, tìm sản phẩm (MASP,TENSP) có giá bán cao nhất.
SELECT NUOCSX, MASP, TENSP FROM (
	SELECT NUOCSX, MASP, TENSP, GIA, RANK() OVER (PARTITION BY NUOCSX ORDER BY GIA DESC) RANK_GIA 
	FROM SANPHAM
) A 
WHERE RANK_GIA = 1
GO
...
FROM (
	SELECT TOP 1 MAKHOA, TENKHOA, RANK() OVER (ORDER BY NGTLAP) RANK_NGTLAP
	FROM KHOA 
) A
WHERE RANK_NGTLAP = 1
GO
...
SELECT HO + ' ' + TEN HOTEN 
FROM LOP 
INNER JOIN HOCVIEN HV ON LOP.TRGLOP = HV.MAHV
WHERE SISO = (
	SELECT MAX(SISO) 
	FROM LOP
)
...
FROM (
	SELECT MAHV, RANK () OVER (ORDER BY COUNT(MAMH) DESC) RANK_MH 
	FROM KETQUATHI KQ 
...
SELECT A.MAMH, TENMH 
FROM (
	SELECT MAMH, RANK() OVER (ORDER BY COUNT(MAHV) DESC) RANK_SOHV 
	FROM KETQUATHI
...
INNER JOIN HOCVIEN HV ON C.MAHV = HV.MAHV
GO
